...
export default function EditorialWorkflows() {
  ...
  const handleImportFromUrl = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/v1/editorial/articles/import-url`, {
        method: 'POST',
        headers: {
  ...
  const fetchData = async () => {
    ...
    try {
      const token = localStorage.getItem('admin_token');
      const artRes = await fetch(`${API_BASE_URL}/api/v1/editorial/articles`, {
      ...
      const clusterRes = await fetch(`${API_BASE_URL}/api/v1/intelligence/clusters`, {
        headers: token ? { 'Authorization': `Bearer ${token}` } : {},
      });
      if (clusterRes.ok) {
  ...
  const handleCreateDraft = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('admin_token');
      const res = await fetch(`${API_BASE_URL}/api/v1/editorial/articles`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          ...(token ? { 'Authorization': `Bearer ${token}` } : {}),
  ...
  const handleGenerateFromCluster = async (clusterId: string) => {
    ...
    try {
      const token = localStorage.getItem('admin_token');
      const res = await fetch(`${API_BASE_URL}/api/v1/editorial/articles/generate-from-cluster`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          ...(token ? { 'Authorization': `Bearer ${token}` } : {}),
  ...
  const handleSaveArticle = async () => {
    ...
    try {
      const token = localStorage.getItem('admin_token');
      const res = await fetch(`${API_BASE_URL}/api/v1/editorial/articles/${editingArticle.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          ...(token ? { 'Authorization': `Bearer ${token}` } : {}),
  ...
  const handleTranslateArticle = async () => {
    ...
    try {
      const token = localStorage.getItem('admin_token');
      const res = await fetch(`${API_BASE_URL}/api/v1/editorial/articles/${showTranslateModal.id}/translate`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          ...(token ? { 'Authorization': `Bearer ${token}` } : {}),
